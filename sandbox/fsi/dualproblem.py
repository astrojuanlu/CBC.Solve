...
def dual_forms(Omega_F, Omega_S, k, problem,
    ...
    A_FM01 =  (1/k)*inner(Z_F, rho_F*tr(dot(grad(v_M),inv(F(U_M1))))*(U_F0 - U_F1))*dx(0)
    A_FM02 =  inner(Z_F, rho_F*tr(dot(grad(v_M),inv(F(U_M1))))*dot(grad(U_F1), dot(inv(F(U_M1)), (U_M1 - U_M0)*(1/k))))*dx_F
    ...
    A_FM05 =  inner(grad(Z_F), tr(dot(grad(v_M),inv(F(U_M1))))*dot(Sigma_F(U_F1, P_F1, U_M1, mu_F), inv(F(U_M1)).T))*dx_F
    ...
    A_FM11 =  inner(Y_F, div(tr(dot(grad(v_M),inv(F(U_M1))))*dot(inv(F(U_M1)), U_F1)))*dx_F
    A_FM12 = -inner(Y_F, div(J(U_M1)*dot(dot(inv(F(U_M1)), grad(v_M)), dot(inv(F(U_M1)), U_F1))))*dx_F

    G_FM1 = -inner(Z_F,  tr(dot(grad(v_M),inv(F(U_M1))))*mu_F*dot(dot(inv(F(U_M1)).T,grad(U_F1).T), dot(inv(F(U_M1)).T, N_F)))*ds
    ...
    A_SM01 = -inner(Z_S('+'), tr(dot(grad(v_M('+')),inv(F(U_M1)('+'))))*dot(dot(Sigma_F(U_F1, P_F1, U_M1, mu_F)('+'),inv(F(U_M1)('+')).T), N))*d_FSI

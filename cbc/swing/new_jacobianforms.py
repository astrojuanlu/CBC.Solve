...
def fsi_jacobian(Iulist,Iudotlist,Iumidlist,U1list,Umidlist,Udotlist,Vlist,dotVlist,matparams,measures,forces,normals):
    ...
    #################################################################
    j_F1 = J_BlockFF(Iu_Fdot,Iu_Fmid,Ip_F,u_Fmid,v_F,dotv_F,q_F,u1_M,rho_F,mu_F,N_F,dxF,dsF,g_F)
    ...
    j_FM = J_BlockFM(u_Fmid, u_Fdot, p1_F, u1_M, Iu_M, v_F, dotv_F, q_F, rho_F, mu_F,N_F, dxF,dsF,g_F)
...
def J_BlockFF(dotdU,dU,dP,U,v,dotv,q,U_M,rho,mu,N_F,dxF,dsF,g_F=None):
    """Fluid Diagonal Block, Fluid Domain """

    #DT (without ALE term)
    A_FF =  inner(dotv, rho*J(U_M)*dotdU)*dxF
    A_FF +=  inner(v, rho*J(U_M)*dot(dot(grad(dU), inv(F(U_M))), U))*dxF
...
def J_BlockFM(U, dotU, P, U_M, dU_M, v_F,dotv, q, rho, mu,N_F, dxF,ds_F,g_F = None):
    """Fluid mesh coupling"""

    #DT (without ALE)
    A_FM =  inner(v_F, rho*J(U_M)*tr(dot(grad(dU_M), inv(F(U_M))))*dotU)*dxF
    A_FM +=  inner(v_F, rho*J(U_M)*tr(dot(grad(dU_M), inv(F(U_M))))*dot(grad(U), dot(inv(F(U_M)), U)))*dxF #No Ale
    A_FM += -inner(v_F,rho*J(U_M)*dot((dot(grad(U), dot(inv(F(U_M)), dot(grad(dU_M), inv(F(U_M)))))), U))*dxF
...
def J_BlockSFbound(dU_F,dP_F,U_M,v_S,mu_F,N_S,dFSI,innerbound):
    ...
    _Sigma_F = PiolaTransform(Sigma_F(dU_F, dP_F, U_M, mu_F), U_M)
    if innerbound == False:
         A_SF = -(inner(dot(_Sigma_F,N_S),v_S))*dFSI
    else:
        A_SF = -(inner(dot(_Sigma_F('+'),N_S('-')),v_S('-')))*dFSI
    return A_SF
...
def J_BlockMM(dUdot_M,dU_M,v_M,dotv_M,mu_M,lmbda_M,dx_F):
    """Mesh diagonal block"""
    _Sigma_M = Sigma_M(dU_M, mu_M, lmbda_M)
    R_M = inner(dotv_M, dUdot_M)*dx_F + 0.5*inner(sym(grad(v_M)), _Sigma_M)*dx_F
    return R_M
